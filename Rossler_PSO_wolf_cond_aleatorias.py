...
class LorenzProblem(Problem):
    def __init__(self):
        super().__init__(n_var=3, n_obj=1, n_ieq_constr=0,
                         n_eq_constr=0, xl=[0.001, 0.001, 0.001], xu=[1, 1, 10])
...
for run in range(0, 10):
    f = open("progress_pso_rossler_polaco" + str(run) + ".txt", 'w')
    original = sys.stdout
    ...
    output_file = "last_gen_pso_rossler_polaco" + str(run) + ".dat"
